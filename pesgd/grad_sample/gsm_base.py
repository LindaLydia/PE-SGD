...
from pesgd.utils.module_utils import trainable_parameters
from torch.utils.hooks import RemovableHandle


logger = logging.getLogger(__name__)

PESGD_PARAM_MONKEYPATCH_ATTRS = [
    "grad_sample",
    "_forward_counter",
...
class AbstractGradSampleModule(nn.Module, ABC):
    ...
    def zero_grad(self, set_to_none: bool = False):
        ...
        if set_to_none is False:
            logger.debug(
                ...
                "PE-SGD will set p.grad_sample to None due to "
    ...
    def _close(self):
        # Clean up attributes
        for attr in PESGD_PARAM_MONKEYPATCH_ATTRS:
            for p in self.parameters():
                if hasattr(p, attr):
                    delattr(p, attr)
