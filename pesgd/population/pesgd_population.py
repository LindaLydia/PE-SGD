...
from pesgd.constant.data import DP_HISTOGRAM_COLUMN_NAME
from pesgd.constant.data import POST_PROCESSED_DP_HISTOGRAM_COLUMN_NAME
from pesgd.constant.data import PARENT_SYN_DATA_INDEX_COLUMN_NAME
from pesgd.constant.data import FROM_LAST_FLAG_COLUMN_NAME
from pesgd.constant.data import VARIATION_API_FOLD_ID_COLUMN_NAME
from pesgd.logging import execution_logger


class PESGDPopulation(Population):
    ...
    def _select_data(self, syn_data, num_samples):
        ...
        if self._selection_mode == "sample":
            ...
            new_data_frame[PARENT_SYN_DATA_INDEX_COLUMN_NAME] = syn_data.data_frame.index[indices]
            return Data(data_frame=new_data_frame, metadata=syn_data.metadata)
        elif self._selection_mode == "random":
            # Randomly select num_samples from the synthetic data
            if len(syn_data.data_frame) < num_samples:
                raise ValueError(
                    f"Not enough samples in synthetic data to select {num_samples} samples. "
                    f"Only {len(syn_data.data_frame)} samples available."
                )
            indices = np.random.choice(len(syn_data.data_frame), size=num_samples, replace=False)
            new_data_frame = syn_data.data_frame.iloc[indices]
            new_data_frame[PARENT_SYN_DATA_INDEX_COLUMN_NAME] = syn_data.data_frame.index[indices]
            return Data(data_frame=new_data_frame, metadata=syn_data.metadata)
        else:
    ...
    def next(self, syn_data, num_samples, selected=False):
        ...
        for variation_api_fold_id in range(self._next_variation_api_fold):
            ...
            # print(f"[debugging] {selected_data.data_frame.columns=}")
