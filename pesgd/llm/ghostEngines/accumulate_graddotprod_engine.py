...
class AccumulateGradDotProdEngine:
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        Args:
            ...
            train_data_size: The number of total train samples we would like to accumulate.
            val_data_size: The number of total validation samples we would like to accumulate.
    ...
    def attach(self, optimizer: torch.optim.Optimizer):
        ...
        accumulate_autograd_grad_sample_dotprod.add_hooks(
            model=self.module,
    ...
    def detach(self):
        ...
        accumulate_autograd_grad_sample_dotprod.remove_hooks(self.module)
    ...
    def prepare_dotprod(self, model: nn.Module, activations: List[torch.Tensor], grad_outputs: List[torch.Tensor], val_size: int, device: str = 'cuda:0') -> torch.Tensor:
        result = None
        train_activation, val_activation = activations[0], activations[-1]
        train_grad_output, val_grad_output = grad_outputs[0], grad_outputs[-1]
        counter = 0
        for name, layer in model.named_modules():
            if hasattr(layer, "activations") and hasattr(layer, "grad_output"):
                # print(f"[debugging] in <./pe/llm/ghostEngines/accumulate_graddotprod_engine.py> 0000 ghost version train, {name=}, {layer=}")
                _activation = torch.concat([train_activation[counter], val_activation[counter]], dim=0).to(device)
                _grad_output = torch.concat([train_grad_output[counter], val_grad_output[counter]], dim=0).to(device)
                accumulate_autograd_grad_sample_dotprod._prepare_sample_dotprod(
                    layer=layer,
                    activation=_activation,  # shape: [#train_sample*2 or #train_sample+#priv_sample, #activation_dim]
                    grad_output=_grad_output, # shape: [#train_sample*2 or #train_sample+#priv_sample, #grad_dim]
                    val_size=val_size,  # = #train_sample*2 / #priv_sample
                )
                # print(f"[debugging] in <./pe/llm/ghostEngines/accumulate_graddotprod_engine.py> ghost version train, prepared sample dot product for layer '{name}' with {layer.weight.grad_dot_prod=} of shape {layer.weight.grad_dot_prod.shape}")
                
                # result.append(layer.weight.grad_dot_prod)
                if result is None:
                    result = layer.weight.grad_dot_prod
                else:
                    result += layer.weight.grad_dot_prod 
                del layer.weight.grad_dot_prod
                layer.weight.grad_dot_prod = None
                ...
                gc.collect()
                # print(f"[debugging] torch.cuda.memory, in prepare_dotprod: {torch.cuda.memory_allocated(0) / 1024 ** 3:.2f} GB, {torch.cuda.memory_reserved(0) / 1024 ** 3:.2f} GB")
