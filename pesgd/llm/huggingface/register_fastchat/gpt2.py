...
Llama3_default_chat_template = """{{- bos_token }}
...
Using the conversation template in the tokenizer, {{- bos_token }}
{%- if custom_tools is defined %}
    {%- set tools = custom_tools %}
{%- endif %}
{%- if not tools_in_user_message is defined %}
...
def register():
    ...
    class Llama3Adapter(BaseModelAdapter):
        """The Llama model adapter for fastchat."""
        ...
        def get_default_conv_template(self, model_path):
            ...
            #     print(f"Using the conversation template in the tokenizer, {tokenizer.chat_template}")
            #     return tokenizer.chat_template
            # else:
            return get_conv_template("llama-3.2")
    ...
    register_conv_template(
        Conversation(
            ...
            stop_str=["<|eot_id|>"],
            messages=[],
            # metadata={"chat_template": Llama3_default_chat_template},
        )
    )
