...
class PRVAccountant(IAccountant):
    ...
    def step(self, *, noise_multiplier: float, sample_rate: float):
        # print(f"[debugging] in <./pesgd/accountants/prv.py> step function, see what the history is before updating, {self.history=}")
        if len(self.history) >= 1:
            ...
            # print(f"[debugging] in <./pesgd/accountants/prv.py> step function, history is empty, adding the first entry with {noise_multiplier=}, {sample_rate=}")
            self.history.append((noise_multiplier, sample_rate, 1))
