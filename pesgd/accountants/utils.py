...
def get_noise_multiplier(
    ...
    **kwargs,
    ...
    """
    print(f"in file <pesgd.accountants.utils> func <get_noise_multiplier>, calculating noise multiplier for target_epsilon={target_epsilon}, target_delta={target_delta}, sample_rate={sample_rate}, epochs={epochs}, steps={steps}, accountant={accountant}")
    ...
    if steps is None:
        ...
        steps = epochs  # modified for PESGD, as we only update once per epoch
    ...
    execution_logger.info(
        f"Determined noise multiplier: {sigma_high} for target_epsilon: {target_epsilon}, target_delta: {target_delta}, sample_rate: {sample_rate}, epochs(steps): {steps}"
    )
    return sigma_high
