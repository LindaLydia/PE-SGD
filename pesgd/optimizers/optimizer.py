...
import torch
from pesgd.optimizers.utils import params
...
class PESGDOptimizer(Optimizer):
    """
    ...
    ``PESGDOptimizer`` assumes that parameters over which it performs optimization belong
    to GradSampleModule and therefore have the ``grad_sample`` attribute.

    On a high level ``PESGDOptimizer``'s step looks like this:
    ...
    Examples:
        ...
        >>> pesgd_optimizer = PESGDOptimizer(
        ...     optimizer=optimizer,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.non_priv_data = None
        self.priv_data = None
        # print(f"[debug] in file <pesgd.optimizers.optimizer> func <__init__>: {kwargs=}")
        self.metric_inverse_epsilon = kwargs.get("metric_inverse_epsilon", 1e-6)
        self.use_eigen = kwargs.get("use_eigen", False)
        self.noise_on_vote = kwargs.get("noise_on_vote", False)
        self.clip_or_normalize = kwargs.get("clip_or_normalize", "normalize")
        print(f"{self.metric_inverse_epsilon=}, {self.use_eigen=}, {self.noise_on_vote=}, {self.clip_or_normalize=}")

        self.avg_approx_grad, self.G_coefficient, self.real_avg_grad = None, None, None
    ...
    def grad_accumulate(self):
        """
        Accumulates per-sample gradients into ``p.summed_grad`` for every parameter ``p``.

        After calling this method, ``p.summed_grad`` will hold a list of sample
        gradients (tensors of shape [max_physical_batch_size, gradient_shape(may be of multiple dimentions)]) 
        over all samples seen since the last optimizer step.
        """
        ...
        for p in self.params:
            ...
            if p.summed_grad is not None:
                p.summed_grad.append(grad_sample)
            else:
                p.summed_grad = [grad_sample]

            _mark_as_processed(p.grad_sample)
    
    def grad_noisy_projection(self):
        """
        Performs gradient projection with added noise.
        """
        # Let G = self.none_priv_grads.T
        # print(f"[debug] in file <pesgd.optimizers.optimizer> func <grad_noisy_projection>: {self.none_priv_grads.shape=}, {self.priv_grads.shape=}")
        # print(f"[debug] in file <pesgd.optimizers.optimizer> func <grad_noisy_projection>: {self.noise_multiplier=}, {self.metric_inverse_epsilon=}")
        G = self.none_priv_grads.T  # shape: [#param, #train_sample]
        eps = self.metric_inverse_epsilon  # small epsilon for numerical stability
        ...
        # print(f"[debugging] see {self.priv_grads=}")

        if self.use_eigen == False:
            ...
            # store the none-noisy average self.priv_grads
            real_average_grad = copy.deepcopy(torch.mean(self.priv_grads, dim=0)) # inner_products.shape=[#val_sample, #param], real_average_grad.shape=[#param]
            # print(f"[debugging] see real averaged gradient {real_average_grad.shape=}")
            execution_logger.info(f"real averaged gradient {real_average_grad.shape=}")

            if not self.noise_on_vote: 
                # add gaussian noise on private sample's clipped gradients: self.priv_grads with shape [#val_sample, #param]
                num_val = self.priv_grads.size(0)
                # step-1, clip and norm
                execution_logger.info(f"{self.priv_grads.norm(dim=1, keepdim=True)=}")
                print(f"self.priv_grads.norm mean={torch.mean(self.priv_grads.norm(dim=1, keepdim=True))}, std={torch.std(self.priv_grads.norm(dim=1, keepdim=True))}, min={torch.min(self.priv_grads.norm(dim=1, keepdim=True))}, max={torch.max(self.priv_grads.norm(dim=1, keepdim=True))}")
                execution_logger.info(f"self.priv_grads.norm mean={torch.mean(self.priv_grads.norm(dim=1, keepdim=True))}, std={torch.std(self.priv_grads.norm(dim=1, keepdim=True))}, min={torch.min(self.priv_grads.norm(dim=1, keepdim=True))}, max={torch.max(self.priv_grads.norm(dim=1, keepdim=True))}")
                # self.priv_grads = self.priv_grads / torch.clamp(self.priv_grads.norm(dim=1, keepdim=True) + 1e-8, min=self.max_grad_norm)  # [#val_sample, #param]
                self.priv_grads = self.priv_grads * (self.max_grad_norm / (self.priv_grads.norm(dim=1, keepdim=True) + 1e-8)).clamp(max=1.0)
                # print(f"[debugging] see clipped normalized {self.priv_grads=}")
                execution_logger.info(f"clipped normalized {self.priv_grads=}")
                # step-2 sum
                self.priv_grads = torch.sum(self.priv_grads, dim=0)  # [#param]
                # print(f"[debugging] see clipped normalized summed {self.priv_grads=}")
                execution_logger.info(f"clipped normalized summed {self.priv_grads=}")
                # step-3 add noise
                self.priv_grads += torch.tensor(np.random.normal(scale=self.noise_multiplier*self.max_grad_norm, size=self.priv_grads.size(0))).to(self.priv_grads.device)
                # print(f"[debugging] see clipped normalized summed and noised {self.priv_grads=}")
                execution_logger.info(f"clipped normalized summed and noised {self.priv_grads=}")
                # step-4 average
                self.priv_grads = self.priv_grads / num_val
                # print(f"[debugging] see averaged clipped normalized summed and noised {self.priv_grads=}")
                execution_logger.info(f"clipped averaged normalized summed and noised {self.priv_grads=}")

                # real_noised_val_grad_cos_sim = torch.dot(real_average_grad, self.priv_grads) / (torch.norm(real_average_grad) * torch.norm(self.priv_grads))
                # real_noised_val_grad_l2_distance = torch.norm(real_average_grad - self.priv_grads)
                # print(f"[debugging] {real_noised_val_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {real_noised_val_grad_l2_distance=}, avg_approximate_grad_norm={torch.norm(self.priv_grads)}, approximate/real norm ration={torch.norm(self.priv_grads)/torch.norm(real_average_grad)}")
                # execution_logger.info(f"{real_noised_val_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {real_noised_val_grad_l2_distance=}, avg_approximate_grad_norm={torch.norm(self.priv_grads)}, approximate/real norm ration={torch.norm(self.priv_grads)/torch.norm(real_average_grad)}")

                # Inner product of each row of self.none_priv_grads and each row of self.priv_grads
                # Result shape: [#train_sample]
                inner_products = self.none_priv_grads @ self.priv_grads.view(-1).T
                ...
                inner_products = self.none_priv_grads @ self.priv_grads.T

                # execution_logger.info(f"[debugging] {GTG.shape=}, {GTG=}")
                # execution_logger.info(f"[debugging] {GTG_inv.shape=}, {GTG_inv=}")
                # execution_logger.info(f"[debugging] {inner_products.shape=}, {inner_products=}")
                # print(f"[debugging] in no-eigen noise on coefficient, {GTG.shape=}, {GTG_inv.shape=}, {inner_products.shape=}")
                # execution_logger.info(f"[debugging] in no-eigen noise on coefficient {GTG.shape=}, {GTG_inv.shape=}, {inner_products.shape=}")


                ################### check real and syn gradient correlation before approximation ###################
                syn_average_grad = copy.deepcopy(torch.mean(self.none_priv_grads, dim=0)) # inner_products.shape=[#val_sample, #param], real_average_grad.shape=[#param]
                # print(f"[debugging] see real averaged gradient {syn_average_grad.shape=}")
                execution_logger.info(f"real averaged gradient {syn_average_grad.shape=}")
                # real_syn_grad_cos_sim = torch.dot(real_average_grad, syn_average_grad) / (torch.norm(real_average_grad) * torch.norm(syn_average_grad))
                # real_syn_grad_l2_distance = torch.norm(real_average_grad - syn_average_grad)
                # print(f"[debug] {real_syn_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {real_syn_grad_l2_distance=}, syn_average_grad_norm={torch.norm(syn_average_grad)}, approximate/real norm ration={torch.norm(syn_average_grad)/torch.norm(real_average_grad)}")
                # execution_logger.info(f"{real_syn_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {real_syn_grad_l2_distance=}, syn_average_grad_norm={torch.norm(syn_average_grad)}, approximate/real norm ration={torch.norm(syn_average_grad)/torch.norm(real_average_grad)}")
                ################### check real and syn gradient correlation before approximation ###################
                ...
                # Result shape: [#train_sample, #val_sample]
                G_coefficient = GTG_inv @ inner_products
                # print(f"[debugging] see non-normalized G_coefficient {G_coefficient=}")
                execution_logger.info(f"non-normalized G_coefficient {G_coefficient=}")

                # add noise on to G_coefficient
                # print(f"[debugging] see {G_coefficient.norm(dim=0, keepdim=True).shape=}, {G_coefficient.norm(dim=0, keepdim=True)=}")
                execution_logger.info(f"{G_coefficient.norm(dim=0, keepdim=True).shape=}, {G_coefficient.norm(dim=0, keepdim=True)=}")
                # print(f"[debugging] see G_coefficient.norm mean={torch.mean(G_coefficient.norm(dim=0, keepdim=True))}, std={torch.std(G_coefficient.norm(dim=0, keepdim=True))}, min={torch.min(G_coefficient.norm(dim=0, keepdim=True))}, max={torch.max(G_coefficient.norm(dim=0, keepdim=True))}")
                ...
                # asser 1 == 0
                if self.clip_or_normalize == 'clip':
                    # G_coefficient = G_coefficient / torch.clamp(G_coefficient.norm(dim=0, keepdim=True) + 1e-8, min=self.max_grad_norm)
                    G_coefficient = G_coefficient * (self.max_grad_norm / (G_coefficient.norm(dim=0, keepdim=True) + 1e-8)).clamp(max=1.0)
                    ...
                    G_coefficient += torch.tensor(np.random.normal(scale=self.noise_multiplier*self.max_grad_norm, size=G_coefficient.size(0))).to(G_coefficient.device)
                    # print(f"[debugging] see summed up and DP-ed {G_coefficient=}")
                    execution_logger.info(f"summed up and DP-ed {G_coefficient=}")
                else:
                    assert self.clip_or_normalize == 'normalize', f"[ERROR] only supporting normalize or clip for {clip_or_normalize=} currently"
                    ...
                    ################### check real and no-noised gradient approximation ###################
                    approximate_grad = G @ G_coefficient # shape: [#param]
                    average_approximate_grad = approximate_grad / inner_products.size(-1) # shape: [#param]
                    # no_noised_real_approx_grad_cos_sim = torch.dot(real_average_grad, average_approximate_grad) / (torch.norm(real_average_grad) * torch.norm(average_approximate_grad))
                    # no_noised_real_approx_grad_l2_distance = torch.norm(real_average_grad - average_approximate_grad)
                    # print(f"[debugging] {no_noised_real_approx_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {no_noised_real_approx_grad_l2_distance=}, avg_approximate_grad_norm={torch.norm(average_approximate_grad)}, approximate/real norm ration={torch.norm(average_approximate_grad)/torch.norm(real_average_grad)}")
                    # execution_logger.info(f"[debugging] {no_noised_real_approx_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {no_noised_real_approx_grad_l2_distance=}, avg_approximate_grad_norm={torch.norm(average_approximate_grad)}, approximate/real norm ration={torch.norm(average_approximate_grad)/torch.norm(real_average_grad)}")
                    ################### check real and no-noised gradient approximation ###################

                    G_coefficient += torch.tensor(np.random.normal(scale=self.noise_multiplier, size=G_coefficient.size(0))).to(G_coefficient.device)
                    # print(f"[debugging] see summed up and DP-ed {G_coefficient=}")
                    execution_logger.info(f"summed up and DP-ed {G_coefficient=}")
            ...
            del G
            del inner_products
            # print(f"[debugging] check {self.none_priv_grads=} after 'del G'")
        else:
            num_syn = G.size(-1)
            ...
            # final grad = V @ V.T @ self.priv_grads
            #            = G @ U @ diag(E)**(-1) @ U.T @ G.T @ self.priv_grads; diag(E)**(-1) is to control the norm
            G_coefficient = ((U @ torch.diag(1/E) @ U.T) @ (G.T @ self.priv_grads))
    ...
    def update_model_update_grads(self, average_approximate_grad, real_average_grad):
        """
        Updates the model gradients using the average approximate gradients and the real average gradients.
        """
        ...
        # print(f"[debug] {real_approximate_grad_cos_sim=}, avg_real_grad_norm={torch.norm(real_average_grad)}, {real_approximate_grad_l2_distance=} avg_approximate_grad_norm={torch.norm(average_approximate_grad)}, approximate/real norm ration={torch.norm(average_approximate_grad)/torch.norm(real_average_grad)}")
        ...
        param_numels = [param.numel() for param in self.params if (hasattr(param, "grad_sample") and param.requires_grad)]
    ...
    def pre_step(
        ...
        """
        Perform actions specific to ``PESGDOptimizer`` before calling
        ...
        # Essentially the PESGDOptimizer act as a normal optimizer
        if self.grad_samples is None or len(self.grad_samples) == 0:
            return True
        # ############# dp-optimizer step pre-processing logic #############
        ...
        # self._is_last_step_skipped = False
        # ############# dp-optimizer step pre-processing logic #############
        # ############# pesgd-optimizer step pre-processing logic #############
        self.save_priv_gradients()
        self.none_priv_grads = self.none_priv_grads.to(self.priv_grads.device)
        ...
        torch.cuda.empty_cache()
        gc.collect()
        self.avg_approx_grad, self.G_coefficient, self.real_avg_grad = self.grad_noisy_projection()
        self.update_model_update_grads(self.avg_approx_grad, self.real_avg_grad)
        # self.scale_grad() # TODO: ???
        # if self.step_hook:
        #     self.step_hook(self)
        self._is_last_step_skipped = False
        # ############# pesgd-optimizer step pre-processing logic #############
    ...
    # def save_priv_gradients(self):
    #     """
    #     Save private gradient gathering for all parameters.
    #     """
    #     self.priv_grads = []
    #     for p in self.params:
    #         if hasattr(p, "summed_grad"):
    #             # print(f"[debug] in file <pesgd.optimizers.optimizer> func <save_priv_gradients> checking {hasattr(p, 'summed_grad')=}, {p.summed_grad=}")
    #             # print(f"[debug] in file <pesgd.optimizers.optimizer> func <save_priv_gradients> {type(p).__name__=} checking {hasattr(p, 'summed_grad')=}, {[_g.shape for _g in p.summed_grad]=}")
    #             grad_this_param = torch.concat(p.summed_grad)
    #             self.priv_grads.append(grad_this_param)
    #             del p.summed_grad
    #             # print(f"[debug] inf file <pesgd.optimizers.optizer> func <save_priv_gradients> checking {grad_this_param=} after 'del p.summed_grad'")
    #             p.summed_grad = None
    #     self.priv_grads = [item.flatten(start_dim=1) for item in self.priv_grads]
    #     self.priv_grads = torch.cat(self.priv_grads, dim=1)
        print(f"{self.priv_grads.shape=}")

    def save_priv_gradients(self):
        r"""Saves the private gradients that are not None for all parameters.

        This is the projection base for the current iteration.

        Returns:
            No return value. Saves the private gradients in
            `self.priv_grads`.
        """
        self.priv_grads = []
        for p in self.params:
            if hasattr(p, "grad_sample") and p.requires_grad:
                grad_this_param = torch.concat(p.grad_sample)
                self.priv_grads.append(grad_this_param)
                del p.grad_sample
                # print(f"[debug] inf file <pesgd.optimizers.optizer> func <save_priv_gradients> checking {grad_this_param=} after 'del p.grad_sample'")
                p.grad_sample = []
        self.priv_grads = [item.flatten(start_dim=1) for item in self.priv_grads]
        self.priv_grads = torch.cat(self.priv_grads, dim=1)
        print(f"{self.priv_grads.shape=}")
    ...
    def save_none_priv_gradients(self):
        r"""Saves the none private gradients that are not None for all parameters.

        This is the projection base for the current iteration.

        Returns:
            No return value. Saves the none private gradients in
            `self.none_priv_grads`.
        """
        self.none_priv_grads = []
        for p in self.params:
            if hasattr(p, "grad_sample") and p.requires_grad:
                grad_this_param = torch.concat(p.grad_sample)
                self.none_priv_grads.append(grad_this_param)
                del p.grad_sample
                # print(f"[debug] inf file <pesgd.optimizers.optizer> func <save_none_priv_gradients> checking {grad_this_param=} after 'del p.grad_sample'")
                p.grad_sample = []
        self.none_priv_grads = [item.flatten(start_dim=1) for item in self.none_priv_grads]
        self.none_priv_grads = torch.cat(self.none_priv_grads, dim=1).to('cpu')
        print(f"{self.none_priv_grads.shape=}")
        torch.cuda.empty_cache()
        gc.collect()

    def get_G_coefficient(self):
        _saved_G_coefficient = copy.deepcopy(self.G_coefficient)
        del self.G_coefficient
        del self.real_avg_grad
        del self.avg_approx_grad
        del self.none_priv_grads
        del self.priv_grads
        torch.cuda.empty_cache()
        gc.collect()
        return _saved_G_coefficient