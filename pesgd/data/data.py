...
class Data:
    ...
    def load_checkpoint(self, path):
        ...
        print(f"{data_frame_path=}, {metadata_path=}")
        print(f"{os.path.exists(data_frame_path)=}, {os.path.exists(metadata_path)=}")
    ...
    def select_by_index(self, selected_index):
        selected = self.data_frame.iloc[list(selected_index)]
        new_obj = copy.deepcopy(self)
        new_obj.data_frame = selected
        return new_obj
    
    def _get_num_classes(self):
        return 0

    def poisson_sampling(self, sampling_ratio):
        """Perform Poisson sampling on the data frame

        :param sampling_ratio: The sampling ratio for Poisson sampling
        :type sampling_ratio: float
        :return: A new :py:class:`pe.data.Data` object with the sampled data frame
        :rtype: :py:class:`pe.data.Data`
        """
        ...
        while len(sampled_indices) == 0:
            random_values = np.random.rand(len(self.data_frame))
            sampled_indices = [i for i, v in enumerate(random_values) if v <= sampling_ratio]
        return self.select_by_index(sampled_indices)
